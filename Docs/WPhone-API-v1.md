...
| `call.incoming` | `caller` | `title`、`callKind` | 通过 CallKit 报告系统来电页 |
...
`call.incoming` 会创建 CallKit 系统来电页，但 WPhone 不建立媒体通道。拒绝会取消 CallKit；接听会结束合成来电并提交一条可进入微信的操作通知。由于 Packet Tunnel Extension 不能直接打开其他 App，用户还需点击该通知的正文或“打开微信”。系统 CallKit 按钮的标题和系统“信息”行为不能由 WPhone 定制。`call.ended` 必须引用对应来电事件的 `id`：
...
- `call_notification_submitted`（已向 CallKit 提交；为保持 v1 响应兼容继续使用此值）
- `notification_removed`
- `logged_only`

`202` 表示 WPhone 已校验、记录并向本地处理流程提交事件，不保证用户一定看到通知。通知权限、系统策略或异步通知提交错误仍可能阻止展示；错误会写入 `debug.log` 和 `/api/status`。
...
- `GET /api/status`：事件计数、最近事件、幂等记录数、监听器和通知状态。

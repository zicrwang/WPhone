...
`call.incoming` 会在约 1 秒后创建 AlarmKit 系统提醒，并独立提交一条可打开微信的 time-sensitive 本地通知作为顶部横幅兜底，但 WPhone 不建立媒体通道。AlarmKit 与顶部横幅读取主 App 中两项独立的最长 10 秒铃声设置；两项的默认资源都是 bundle 内的 `WPhoneIncomingCall.wav`。系统页的“拒绝”和通知的“关闭”都会停止提醒；“接听/打开”会停止提醒、启动 WPhone，再由主 App 打开微信。`callKind` 在 v1 中保留为结构化元数据，不改变 AlarmKit 展示。当前只保留一个活动来电提醒，新的 `call.incoming` 会替换前一个；没有收到匹配的 `call.ended` 或用户关闭操作时，提醒在触发 50 秒后自动停止并清理横幅。`call.ended` 必须引用对应来电事件的 `id`：

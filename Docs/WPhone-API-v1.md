...
`call.incoming` 会在约 1 秒后创建 AlarmKit 系统提醒，但 WPhone 不建立媒体通道。系统页的“拒绝”停止提醒；“接听”停止提醒、启动 WPhone，再由主 App 打开微信。`callKind` 在 v1 中保留为结构化元数据，不改变 AlarmKit 展示。当前只保留一个活动来电提醒，新的 `call.incoming` 会替换前一个。`call.ended` 必须引用对应来电事件的 `id`：

...
| 字段 | 值 | 默认值 |
| --- | --- | --- |
| `priority` | `normal`、`timeSensitive` | `normal` |
| `sound` | `default`、`none` | `default` |

`timeSensitive` 仍受 iOS 通知权限、专注模式和系统策略控制，不代表 Critical Alert。
为保持微信普通消息式的外观，`call.incoming` 的并行兜底横幅固定使用 `normal`（iOS `.active`）；即使旧客户端仍传入 `timeSensitive`，该字段也只作为兼容输入接受，不改变来电横幅级别。其他会显示通知的事件继续遵循 `delivery.priority`。
对 `call.incoming`，`sound` 只控制并行提交的本地通知横幅；AlarmKit 始终使用项目配置的来电声音，因为 AlarmKit 没有静音 `AlertSound` 选项。
...
| `call.incoming` | `caller` | `title`、`callKind` | 调度 iOS 26 AlarmKit，并提交普通 `active` 本地通知横幅 |
...
`call.incoming` 会在约 1 秒后创建 AlarmKit 系统提醒，并独立提交一条可打开微信的普通 `active` 本地通知作为顶部横幅兜底，但 WPhone 不建立媒体通道。普通级别不显示系统“时效通知”标签，也不能越过专注模式或通知摘要。AlarmKit 与顶部横幅读取主 App 中两项独立的铃声设置；闹钟文件最长 60 秒，横幅文件最长 29 秒，两项的默认资源都是 bundle 内的 `WPhoneIncomingCall.wav`。系统页的“拒绝”和通知的“关闭”都会停止提醒；“接听/打开”会停止提醒、启动 WPhone，再由主 App 打开微信。`callKind` 在 v1 中保留为结构化元数据，不改变 AlarmKit 展示。当前只保留一个活动来电提醒，新的 `call.incoming` 会替换前一个；没有收到匹配的 `call.ended` 或用户关闭操作时，提醒在触发 50 秒后自动停止并清理横幅。`call.ended` 必须引用对应来电事件的 `id`：
...
- `alarmkit_schedule_requested`
- `notification_removed`
- `logged_only`

`202` 表示 WPhone 已校验、记录并向本地处理流程提交事件，不保证用户一定看到提醒。AlarmKit 权限、通知权限、专注模式、通知摘要或异步提交错误仍可能阻止或延迟展示；普通 `active` 来电横幅与 AlarmKit 并行提交，AlarmKit 错误会写入 `debug.log` 和 `/api/status`。
...
- `GET /api/status`：事件计数、最近事件、幂等记录数、监听器、中继、通知和 `alarmKit` 状态。`relay` 包含出站中继连接状态、地址、端口和已收事件数。`alarmKit.hostAuthorization` 是主 App 写入的最近授权状态，`alarmKit.extensionAuthorization` 是 Packet Tunnel 当前读取值；两者可能不同。

...
| --- | --- | --- | --- |
| `message.received` | `body` | `title`、`sender`、`conversationId`、`mediaKind` | 显示一条独立本地通知 |
| `call.incoming` | `caller` | `title`、`callKind` | 显示电话样式的本地通知 |
| `call.ended` | `targetId` | 其他字段被忽略 | 移除同一 `source` 下、ID 为 `targetId` 的通知 |
...
`call.incoming` 不是 CallKit 来电。它不会建立通话，也不能保证持续响铃。`call.ended` 必须引用对应来电事件的 `id`：

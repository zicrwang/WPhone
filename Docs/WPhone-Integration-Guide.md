...
| --- | --- | --- | --- |
| 微信 | `wechat.<设备名>` | 微信图标 | 显示全屏过渡页后打开微信 |
...
该映射适用于 `message.received`、`notification.show` 和 `call.incoming`。`wechat` 是唯一有外部跳转的前缀，目标固定为 `weixin://`；点按后会先显示不含设置选项的全屏微信过渡页约 0.9 秒。不能在 JSON 中传图片、包名、Deep Link 或 URL 来改变图标、打开目标或 App 图标。`source` 是展示选择和幂等键，不是认证字段。
...
| --- | --- | --- | --- |
| 收到文本、图片或其他消息 | `message.received` | `body` | 选择与上游 App 对应的 `source`；将联系人放入 `sender`，会话 ID 放入 `conversationId`；成功提交后 5 秒自动清理 |
...
| 显示不属于消息/来电的通用提醒 | `notification.show` | `body` | 监控告警、自动化提醒等使用该类型；成功提交后 5 秒自动清理 |
| 移除一条通用提醒 | `notification.dismiss` | `targetId` | 引用对应 `notification.show` 的 `id` |
| 暂无内置语义的厂商事件 | `custom.<vendor>.<name>` | 无 | 当前只写日志，不能期待弹出通知 |

`conversationId`、`mediaKind` 和 `callKind` 当前只是保留的结构化元数据。`message.received` 与 `notification.show` 是普通通知，均在成功提交后 5 秒自动清理，发送端不能配置时长；`notification.dismiss` 只能提前移除对应的通用通知。来电不会建立媒体通道。每个 `call.incoming` 都会提交一条时效本地通知，不需要 APNs；发送端没有发出 `call.ended` 时，WPhone 会在提交后 30 秒自动清理该通知。
...
普通局域网软件通常只能识别“疑似来电通知”，不一定能可靠获得通话生命周期。无法确认结束事件时，不要伪造 `call.ended`；WPhone 会在提交后 30 秒自动移除未关闭的通知。`call.incoming` 只会提交时效通知：“关闭”移除通知，`wechat` 来源的“打开”会先显示全屏微信过渡页约 0.9 秒再进入微信，其他来源的“打开”只进入 WPhone。它不是实际 VoIP 通话，也不传输音频。
...
6. 普通通知后检查 `notifications.regularNotificationAutoClearSeconds` 是否为 `5`；来电后检查 `notifications.incomingCallInterruptionLevel` 是否为 `timeSensitive`，并确认 `incomingCallAutoClearSeconds` 为 `30`。
7. 使用 `/api/logs?cursor=0` 查看 `EVENT_MESSAGE_RECEIVED notification submitted`、`REGULAR_NOTIFICATION_AUTO_CLEARED`、`EVENT_TIME_SENSITIVE_INCOMING notification submitted` 或通知提交错误。

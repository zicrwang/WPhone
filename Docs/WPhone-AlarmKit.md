...
- 来电兜底横幅使用普通 `active` 级别，不需要 Time Sensitive Notifications capability 或 APNs。工程仍保留 `com.apple.developer.usernotifications.time-sensitive` entitlement，用于兼容显式请求 `timeSensitive` 的通用消息事件。
...
WPhone 只创建 `AlarmPresentation.Alert`，不设置 `Alarm.CountdownDuration`，也不包含 Widget/自定义 Live Activity 或应用自己的全屏来电页。每次来电还会并行提交 `interruptionLevel = .active` 的普通本地通知，因为公开 API 不能可靠读取锁屏状态，也不能确认 AlarmKit UI 是否实际展开。普通级别采用与常规消息相同的系统外观，不显示“时效通知”标签，但不能越过专注模式或通知摘要：
...
- 应用只能请求 `.banner` 呈现，不能把横幅风格强制改为“持续”。请在主 App 点击“通知设置”，由用户打开横幅和声音，并把横幅风格选为“持续”。
...
主 App 分为“闹钟铃声”和“顶部横幅铃声”两个区域。每项都可从系统文档选择器打开一份 WAV、CAF 或 AIFF，并可单独恢复内置；选择器使用复制模式，只展示支持的文件类型，选中文件后直接返回 WPhone。闹钟文件最长 60 秒，顶部横幅文件最长 29 秒，每个文件不超过 20 MB，并校验 Linear PCM、IMA4、µLaw 或 aLaw 编码。两项以不同文件名存入 App Group 的 `Library/Sounds`，普通通知直接读取横幅文件；AlarmKit 调度前只把闹钟文件复制到当前主 App 或 Packet Tunnel 数据容器的 `Library/Sounds`。旧版本的单一自定义铃声首次读取时会复制为两项。某项自定义资源丢失或复制失败时，该项独立回退到内置声音，内置资源也缺失时再回退到系统默认声音。
...
| `soundDurationSeconds` | 当前闹钟铃声时长 |
| `maximumSoundDurationSeconds` | 闹钟铃声文件上限，当前为 60 |
| `openBehavior` | 当前为 `open-wphone-then-wechat` |

`notifications` 对象的 `incomingCallInterruptionLevel` 固定为 `active`。`incomingCallSound`、`incomingCallSoundAvailable`、`incomingCallSoundCustom`、`incomingCallSoundDurationSeconds` 和 `incomingCallSoundMaximumDurationSeconds` 描述顶部横幅的独立铃声；横幅文件上限当前为 29 秒。`timeSensitiveSetting` 仅表示显式时效通用消息能否工作，与来电兜底横幅无关；`alertStyle` 为 `persistent` 表示用户选择了持续横幅，为 `temporary` 表示临时横幅。应用只能读取这些系统设置，不能修改。

`GET /api/status` 的 `relay` 对象包含中继连接状态、主机、端口、连接时间和已接收事件数。Armbian 上的 `GET /health` 返回 `providers: 1` 才表示 iPhone 通道已经注册。

所有调度、停止和失败信息都追加写入 App Group 的 `debug.log`，可通过 `GET /api/logs?cursor=<cursor>` 增量读取。Packet Tunnel 不再因自身读到 `not-determined` 而提前退出，而是直接调用 `schedule`；失败日志包含 NSError 的 domain、code、description、可用的 userInfo、调用前后的扩展授权状态和主 App 授权状态。带“打开”操作的普通 `active` 本地通知与 AlarmKit 并行提交，不以 AlarmKit 返回失败为前提。

## 真机验收

1. 安装重签后的完整 IPA，确认 `PacketTunnel.appex` 一同安装。
2. 启动 WPhone，允许通知、AlarmKit 和 VPN 权限；从“通知设置”进入系统页，打开横幅和声音，需要时把横幅风格改为“持续”。可为“闹钟铃声”选择不超过 60 秒的受支持音频，为“顶部横幅铃声”选择不超过 29 秒的受支持音频。
...
6. 向中继站发送 `call.incoming`，确认约 1 秒内出现系统提醒并播放自定义声音；锁定状态验收系统锁屏闹铃界面，已解锁且正在使用时验收 Dynamic Island 或普通 `active` 顶部横幅，确认横幅没有“时效通知”标签，并确认 WPhone 没有自行显示全屏页面。
7. 点击“拒绝”，确认提醒立即停止。

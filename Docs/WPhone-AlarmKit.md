...
6. 向中继站发送 `call.incoming`，确认约 1 秒后出现系统提醒并响铃；锁屏验收系统锁屏闹铃界面，亮屏验收屏幕上方提醒。
7. 点击“拒绝”，确认提醒立即停止。
...
AlarmKit 的视觉样式、声音、持续时间和系统调度由 iOS 控制。它会越过静音和专注模式，但不能保证 VPN Extension 永久存活。Apple 的公开示例从主 App 调度 AlarmKit，并没有明确支持任意 App Extension 调度。主 App 测试成功只证明 AlarmKit 基础配置正确；Packet Tunnel 路径仍必须以上述 iOS 26.x 真机结果作为最终验收依据。

...
WPhone 当前只保留一条活动 AlarmKit 来电。新来电会停止上一条；`call.ended` 只有在 `source + targetId` 与活动来电匹配时才停止它。`POST /api/debug/stop` 和 `POST /STOP_RING` 会停止活动提醒。停止 VPN 只停止隧道和局域网监听，不再取消已经调度给系统的提醒。
...
6. 在调试后台点击“AlarmKit 来电”，确认约 1 秒后出现系统提醒并响铃。
7. 点击“关闭”，确认提醒立即停止。
